...
static int lcurl_multi_socket_action(lua_State *L){
  lcurl_multi_t *p = lcurl_getmulti(L);
  curl_socket_t s  = (curl_socket_t)lutil_optint64(L, 2, CURL_SOCKET_TIMEOUT);
  CURLMcode code; int n, mask;
  if(s == CURL_SOCKET_TIMEOUT) mask = (int)lutil_optint64(L, 3, 0);
  else mask = (int)lutil_checkint64(L, 3);
...
int lcurl_multi_timer_callback(CURLM *multi, long ms, void *arg){
  ...
  if(lua_gettop(L) > top){
    ...
    else ret = (int)lua_tointeger(L, top + 1);
  }
