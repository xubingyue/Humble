...
typeclosure(lua_State *L) {
    ...
    int i;
    int n = sizeof(actypename) / sizeof(actypename[0]);
    for (i = 0; i < n; i++) {
        lua_pushstring(L, actypename[i]);
    }
...
init_oid_header() {
    ...
    uint32_t c = h ^ (uint32_t)time(NULL) ^ (uintptr_t)&h;
    if (c == 0) {
        c = 1;
    }
    oid_counter = c;
...
lobjectid(lua_State *L) {
    ...
    if (lua_isstring(L, 1)) {
        ...
        uint32_t id = H_AtomicAdd(&oid_counter, 1);
...
luaopen_bson(lua_State *L) {
    ...
    char minkey[] = { (char)0, (char)BSON_MINKEY };
