...
static inline void
buffer_destroy(struct lbuffer *b) {
	if (b->ptr != b->buffer) {
		free(b->ptr);
	}
}
...
static inline void
buffer_create(struct lbuffer *b) {
	b->size = 0;
	b->cap = DEFAULT_CAP;
	b->ptr = b->buffer;
}
...
static inline void
buffer_reserve(struct lbuffer *b, int sz) {
	if (b->size + sz <= b->cap)
		return;
	do {
		b->cap *= 2;
	...
	if (b->ptr == b->buffer) {
        ...
        uint8_t *ptmp = (uint8_t *)realloc(b->ptr, b->cap);
        H_ASSERT(NULL != ptmp, "realloc error.");
		b->ptr = ptmp;
	}
}

static inline void
write_int32(struct lbuffer *b, int32_t v) {
...
static inline void
write_bytes(struct lbuffer *b, const void * buf, int sz) {
	buffer_reserve(b,sz);
...
static void
write_string(struct lbuffer *b, const char *key, size_t sz) {
...
static inline int
reserve_length(struct lbuffer *b) {
	int sz = b->size;
	buffer_reserve(b,4);
	b->size +=4;
	return sz;
...
static inline void
write_length(struct lbuffer *b, int32_t v, int off) {
	uint32_t uv = (uint32_t)v;
	b->ptr[off++] = uv & 0xff;
	b->ptr[off++] = (uv >> 8)&0xff;
...
//		int32_t length; // total message size, including this
    ...
    const streply* reply = (const streply *)data;
